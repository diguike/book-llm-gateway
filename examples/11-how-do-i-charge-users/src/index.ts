...
//
// 相对 v0.9 的核心变化:
//   1. 新增 src/optimization/ 子系统:
//        - prompt-cache.ts  : 上游 usage 缓存命中字段抽取 (Anthropic / OpenAI / DeepSeek)
//        - cost-router.ts   : 按 cost_priority asc 选最便宜可用 channel + fallback
//        - batch-channel.ts : priority=low 请求走 batch 异步队列, 用 supports_batch=true channel
//   2. drizzle/0006_cost_optimization.sql: channels 加 cost_priority/supports_batch;
//      prices 加 cache_input/batch_input/batch_output 三列单价;
//      usage_records 加 cached_input_tokens/cache_write_tokens/batch_mode 三列结算字段.
//   3. 主路径选 channel 改用 pickCheapestChannel (优先 cost asc), 不再用 pickChannelForModel.
//      v0.9 的 pickChannelForModel 保留, 给 /v1/messages 透传路径继续用.
//   4. billing/calculator.ts: postConsume / postConsumeStream 接收 cached/batch 字段, 按 cache
//      单价分拆计费 (computeFinalCost). v0.9 不传这些字段时行为不变.
//   5. 主路径在解析 usage 后, 抽 cached_input_tokens 写回 usage_records, 走 cache 单价.
//   6. priority="low" 的请求被路由到 batch-channel 异步处理 (5s 窗口或攒满 10 条触发 flush).
//
// 五件合法降本机制怎么落到代码:
//   - prompt cache 透传:  prompt-cache.ts + 不重写 prompt prefix
//   - batch API 通道:    batch-channel.ts + supports_batch=true 的 channel
//   - 渠道倍率 / 套利:    channels.channel_multiplier (v0.5 已有) + cost_priority (v0.10 新增)
//   - 成本路由 fallback:  cost-router.ts::pickCheapestChannel
//   - 企业渠道价差:       通过 cost_priority + channelMultiplier 两个字段组合表达
//
// 灰色玩法 (本章只揭原理, 不实现):
//   - 拼车号池 / 退款套利 / 偷偷降级模型: 看 tools/model-fingerprint-cli/ 做防御侧检测.
//
// 仍然故意暴露的缺陷 (留给后续章节):
//   - 用户钱包 / 支付链路不存在, 余额是 admin 手动充 (v0.11);
//   - 真实 batch API 是 jsonl 文件上传 + 24h 异步, 本章简化为 5s 窗口 + 同步发,
//     只演示「按 batch 单价计费」的链路, 不接 OpenAI Files + Batches API 完整流程.
...
// v0.11: 启动 PaymentAdaptor. 本章只挂 mock; 真实接入参考附录 A 的 Stripe / 支付宝 / 微信支付.
...
bootstrapPaymentAdaptors({
  mock: {
    baseUrl: MOCK_PAY_BASE_URL,
    notifyUrl: `${PAYMENT_NOTIFY_BASE}/payment/notify/mock`,
    secret: MOCK_PAY_SECRET,
  },
});
bootLogger.info(
  { mock_pay_base: MOCK_PAY_BASE_URL, notify_base: PAYMENT_NOTIFY_BASE },
