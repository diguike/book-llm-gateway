...
import { keys, orgs, users, prices, usageRecords, recharges, refunds, wallets } from '../db/schema.js';
...
import { getPaymentAdaptor, listPaymentAdaptors } from '../payment/registry.js';
...
export function createAdminRouter(): Hono {
  ...
  app.post('/users', async (c) => {
    ...
    // v0.11: 同时建一个钱包, 把 balanceCny 注入. users.balance_micro 还在,
    //        但实际扣减走 wallets.balance_micro. 老代码兼容字段不破坏.
    const wallet = ensureWallet({
      userId: rows[0]!.id,
      initialBalanceMicro: balanceMicro,
    });
  ...
  // ============================================================
  // v0.8: channels CRUD + 健康检查手动触发
  //
  //   GET    /admin/channels                  列出全部 channel (含 disabled / probing)
  //   POST   /admin/channels                  创建 channel
  //   PATCH  /admin/channels/:id              修改 channel
  //   DELETE /admin/channels/:id              删除 channel
  //   POST   /admin/channels/:id/disable      手动禁用 (运营拉闸用)
  //   POST   /admin/channels/:id/enable       手动恢复
  //   POST   /admin/channels/probe-now        立刻跑一次 health-checker
  //
  // 每个写操作都会同步刷新 abilities 表 + ChannelRegistry 内存索引,
  // 改完后下一个请求就生效, 不需要重启进程.
  // ============================================================
  app.get('/channels', async (c) => {
    ...
    return c.json({
      data: snap.map((s) => ({
        ...
        name: s.name,
        provider: s.provider,
        // 注: api_key 不能直接返出去, 截前 6 + 后 4 做预览
        api_key_preview: maskKey(s.apiKey),
        base_url: s.baseUrl,
        enabled_models: s.enabledModels,
        group: s.groupName,
        priority: s.priority,
        weight: s.weight,
        channel_multiplier: s.channelMultiplier / 1000,
        status: s.status,
      })),
    });
  });
  ...
  app.post('/channels', async (c) => {
    const schema = z.object({
      ...
      channelMultiplier: z.number().positive().max(10).optional(),
    });
    const parsed = schema.safeParse(await c.req.json().catch(() => null));
    if (!parsed.success) {
      return c.json({ error: { message: 'invalid_request', detail: parsed.error.format() } }, 400);
    ...
    const ch = createChannel({
      ...
      groupName: parsed.data.group,
      priority: parsed.data.priority,
      weight: parsed.data.weight,
      channelMultiplier: parsed.data.channelMultiplier,
    });
    return c.json(
      {
        id: ch.id,
        ...
        api_key_preview: maskKey(ch.apiKey),
        base_url: ch.baseUrl,
        enabled_models: JSON.parse(ch.enabledModels),
        group: ch.groupName,
        priority: ch.priority,
        weight: ch.weight,
        channel_multiplier: ch.channelMultiplier / 1000,
        status: ch.status,
      },
      201,
    );
  ...
  app.patch('/channels/:id', async (c) => {
    ...
    const schema = z.object({
      ...
      enabledModels: z.array(z.string().min(1)).min(1).optional(),
      group: z.string().optional(),
      priority: z.number().int().optional(),
      weight: z.number().int().nonnegative().optional(),
      channelMultiplier: z.number().positive().max(10).optional(),
    ...
    if (!updated) {
      return c.json({ error: { message: `channel ${id} not found` } }, 404);
    }
  ...
  app.delete('/channels/:id', async (c) => {
    ...
    if (!ok) return c.json({ error: { message: `channel ${id} not found` } }, 404);
  ...
  app.post('/channels/:id/disable', async (c) => {
    ...
    const reason = parsed.success ? parsed.data.reason ?? 'manual_disable' : 'manual_disable';
    markChannelDisabled(id, reason);
    return c.json({ id, status: 'disabled', reason });
  });

  app.post('/channels/:id/enable', async (c) => {
    const id = Number(c.req.param('id'));
    markChannelActive(id);
    return c.json({ id, status: 'active' });
  });

  app.post('/channels/probe-now', async (c) => {
    const result = await runHealthCheckOnce({ probeTimeoutMs: 10_000 });
  ...
  // v0.11 新增: 钱包 / 充值 / 退款管理
  //
  //   GET    /admin/wallets/:userId               查 wallet
  //   POST   /admin/wallets/:userId/seed          手动建 wallet (兼容老 user)
  //   POST   /admin/recharges                     创建充值订单 (返回支付凭据)
  //   POST   /admin/recharges/:id/admin-credit    管理员强制入账 (跳过支付平台, 应急用)
  //   GET    /admin/recharges?userId=             列充值流水
  //   POST   /admin/refunds                       发起退款 (调 adapter.refund)
  //   GET    /admin/refunds?rechargeId=           列退款流水
  //   GET    /admin/payment-providers             列已注册的支付适配器
  // ============================================================
  ...
  app.get('/wallets/:userId', async (c) => {
    ...
    if (!wallet) {
      return c.json({ error: { message: `wallet for user ${userId} not found` } }, 404);
    }
    return c.json({
      id: wallet.id,
      ...
      frozenMicro: wallet.frozenMicro,
  ...
  /**
   * 管理员强制设置 wallet 余额. 仅供测试 / 应急使用. 真实生产不应该暴露这个接口
   * (会破坏「所有金额变动有据可查」的不变量).
   */
  app.post('/wallets/:userId/admin-set', async (c) => {
    const userId = Number(c.req.param('userId'));
    if (!Number.isInteger(userId) || userId <= 0) {
      return c.json({ error: { message: 'invalid userId' } }, 400);
    }
    const schema = z.object({ balanceMicro: z.number().int() });
    const parsed = schema.safeParse(await c.req.json().catch(() => null));
    if (!parsed.success) {
      return c.json({ error: { message: 'invalid_request', detail: parsed.error.format() } }, 400);
    }
    ...
    db.update(wallets)
      .set({
        balanceMicro: parsed.data.balanceMicro,
        updatedAt: now,
      })
      .where(eq(wallets.id, wallet.id))
      .run();
    return c.json({ userId, balanceMicro: parsed.data.balanceMicro });
  });

  app.post('/wallets/:userId/seed', async (c) => {
    const userId = Number(c.req.param('userId'));
    if (!Number.isInteger(userId) || userId <= 0) {
      return c.json({ error: { message: 'invalid userId' } }, 400);
    }
    const schema = z.object({ initialBalanceCny: z.number().nonnegative().optional() });
    const parsed = schema.safeParse(await c.req.json().catch(() => ({})));
    const initial = parsed.success ? parsed.data.initialBalanceCny ?? 0 : 0;
    const wallet = ensureWallet({
      userId,
      initialBalanceMicro: Math.round(initial * 1_000_000),
    });
    return c.json({
      id: wallet.id,
      userId: wallet.userId,
  ...
  // 创建充值订单
  //
  //   body: { userId, provider, amountCny }
  //   resp: { recharge: {...}, credential: {...} }
  //
  //   流程:
  //     1. 校验 user / wallet 存在;
  //     2. 网关侧生成 out_trade_no (核心: 全平台唯一);
  //     3. 调 adapter.createOrder() 拿支付凭据 (URL / 二维码);
  //     4. 写一行 recharges (status=pending), 等待平台异步回调;
  //     5. 把支付凭据返给客户端, 客户端去支付.
  // ============================================================
  app.post('/recharges', async (c) => {
    const schema = z.object({
      userId: z.number().int().positive(),
      provider: z.string().min(1),
    ...
    const wallet = getWalletByUserId(parsed.data.userId);
    if (!wallet) {
      return c.json({ error: { message: `user ${parsed.data.userId} has no wallet` } }, 404);
    }
    const adaptor = getPaymentAdaptor(parsed.data.provider);
    if (!adaptor) {
      return c.json(
        {
          error: {
            message: `payment provider ${parsed.data.provider} not registered`,
            available: listPaymentAdaptors(),
          },
        },
        400,
      );
    }
    const outTradeNo = generateOutTradeNo(parsed.data.userId);
    const amountMicro = Math.round(parsed.data.amountCny * 1_000_000);

    let createOutput;
    try {
      createOutput = await adaptor.createOrder({
        outTradeNo,
        amountMicro,
        userId: parsed.data.userId,
        subject: `Top-up ¥${parsed.data.amountCny}`,
      });
    } catch (err) {
      return c.json(
        { error: { message: 'create_order_failed', detail: (err as Error).message } },
        502,
      );
    }
    const recharge = createRechargeOrder({
      userId: parsed.data.userId,
      outTradeNo,
      provider: parsed.data.provider,
      amountMicro,
    ...
    return c.json(
      {
        recharge: {
          ...
          amountMicro: recharge.amountMicro,
          status: recharge.status,
        },
        credential: createOutput.credential,
      },
      201,
    );
  });
  ...
  app.get('/recharges', async (c) => {
    ...
    const limit = c.req.query('limit') ? Math.min(Number(c.req.query('limit')), 500) : 50;
    const db = getDb();
    const q = db.select().from(recharges).orderBy(desc(recharges.createdAt)).limit(limit);
    const rows = userId !== undefined ? q.where(eq(recharges.userId, userId)).all() : q.all();
    return c.json({ data: rows });
  });
  ...
  // 发起退款
  //
  //   body: { rechargeId, reason? }
  //   resp: { refund, accepted }
  //
  //   流程:
  //     1. 校验 recharges 存在 + status=succeeded;
  //     2. 网关侧生成 refund_no;
  //     3. 调 adapter.refund() (大多数平台同步返回「受理成功」, 真实退款是回调里再确认);
  //     4. 写一行 refunds (status=pending);
  //     5. 等 mock 平台异步回调 (POST /payment/notify/:provider, payload.kind=refund);
  //        回调进入 handleRefundCallback, 真正回退余额.
  // ============================================================
  app.post('/refunds', async (c) => {
    const schema = z.object({
      rechargeId: z.number().int().positive(),
    ...
    const db = getDb();
    const rows = db.select().from(recharges).where(eq(recharges.id, parsed.data.rechargeId)).all();
    if (rows.length === 0) {
      return c.json({ error: { message: `recharge ${parsed.data.rechargeId} not found` } }, 404);
    }
    ...
    if (recharge.status !== 'succeeded') {
      return c.json(
        {
          error: {
            message: `recharge status is ${recharge.status}, only 'succeeded' can be refunded`,
          },
        },
        409,
      );
    }
    const adaptor = getPaymentAdaptor(recharge.provider);
    if (!adaptor) {
      return c.json(
        { error: { message: `payment provider ${recharge.provider} not registered` } },
        500,
      );
    }
    const refundNo = generateRefundNo(recharge.id);
    let refundResp;
    try {
      refundResp = await adaptor.refund({
        outTradeNo: recharge.outTradeNo,
        refundNo,
        amountMicro: recharge.amountMicro,
        reason: parsed.data.reason ?? 'user requested refund',
      });
    } catch (err) {
      return c.json(
        { error: { message: 'refund_call_failed', detail: (err as Error).message } },
        502,
      );
    }
    const refund = createRefundOrder({
    ...
    return c.json(
      {
        refund: {
          ...
          rechargeId: refund.rechargeId,
          amountMicro: refund.amountMicro,
          status: refund.status,
        },
        accepted: refundResp.accepted,
      },
      201,
    );
  });
  ...
  app.get('/refunds', async (c) => {
    const rechargeIdRaw = c.req.query('rechargeId');
    const rechargeId = rechargeIdRaw ? Number(rechargeIdRaw) : undefined;
    const limit = c.req.query('limit') ? Math.min(Number(c.req.query('limit')), 500) : 50;
    const db = getDb();
    const q = db.select().from(refunds).orderBy(desc(refunds.createdAt)).limit(limit);
    const rows = rechargeId !== undefined ? q.where(eq(refunds.rechargeId, rechargeId)).all() : q.all();
    return c.json({ data: rows });
  });

  app.get('/payment-providers', async (c) => {
    return c.json({ data: listPaymentAdaptors() });
  });

  return app;
}

/** 把 api_key 脱敏 -- 前 6 + 后 4, 中间 *** */
