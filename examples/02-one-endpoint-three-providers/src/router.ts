// 模型路由表
//
// v0.2 的路由策略: 按 model 字段的前缀决定使用哪个 Adaptor.
//   - gpt-*      / o1-* / o3-*  -> OpenAI
//   - deepseek-*                -> DeepSeek
//
// 路由表通过 (prefix, adaptor) 配对维护. 命中第一个匹配的前缀即使用对应 adaptor.
// 后续章节会演进:
//   - Ch3: 增加 claude-* -> Anthropic, 引入异构协议;
//   - Ch8: 同一个 model 可能映射到多个 Channel (同 provider 多 Key 或多 provider
//          的等价模型), 路由从「prefix -> adaptor」升级为「model -> channels -> adaptor」.
//
// 这一版的路由刻意做得简单: 一张静态表, 不接数据库. 等 Ch4 引入持久层之后,
// 渠道信息会迁移到 DB, 表本身的结构基本不变.
