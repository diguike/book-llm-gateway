...
function main(): void {
  ...
  if (userId === undefined) {
    ...
    // v0.5: 创建 user 时给初始余额, 否则首次调网关立刻 402
    const initialBalanceCny = Number(process.env.INITIAL_BALANCE_CNY ?? 100);
    const userInserted = db
      ...
      .values({
        ...
        name: args.user!,
        createdAt: now,
        balanceMicro: Math.round(initialBalanceCny * 1_000_000),
        userMultiplier: 1000,
      })
      .returning()
      .all();
